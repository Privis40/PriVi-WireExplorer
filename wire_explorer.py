...
class WireExplorerSentinel:
    ...
    def carve_intel(self, pkt):
        ...
        except Exception: return "[grey]Binary Stream.[/grey]"
...
def authorization_gate():
    """Mandatory authorization acknowledgment before tool runs."""
    ...
    gate_text.append(
        "  This tool must only be used on systems or networks you own\n"
        "  or have explicit written authorization to assess.\n\n",
        style="white"
    )
    gate_text.append("    [+]  You own the target, OR\n", style="green")
    gate_text.append("    [+]  You hold a signed Letter of Authorization (LoA).\n\n", style="green")
    gate_text.append(
        "  Unauthorized use is illegal under the Computer Misuse Act,\n"
    ...
    console.print(Panel(gate_text, border_style="red",
                        title="[bold red]PriVi-WireExplorer | PriViSecurity[/bold red]"))
    console.print("[bold white]Do you have authorization to use this tool on the target?[/bold white]")
    ...
    try:
        ...
        _sys.exit(0)
    console.print("\n[bold green][+] Authorization confirmed. Launching....[/bold green]\n")
    _time.sleep(0.8)
